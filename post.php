<?php
	...
	function ErrorHandler ($number, $description, $file, $line)
	{
		if (error_reporting () & $number)
		{
			...
			global $recorded;
			if ($recorded)
				echo "Dein Post konnte zwar eingetragen, doch mˆglicherweise nicht an die Clients gesendet werden!<br>";
			else
				echo "Dein Post konnte mˆglicherweise nicht eingetragen werden!<br>";
...
//	$post["message"]=str_ireplace(array("penis","schwanz","schw√§nze"," arsch","fuck","fick","pisse","drecksau","===D","jerk off","wichs","soggy biscuit"),array("darij","darij","darijs"," kreisquadrierer","freu","freu","pizza"," Fermatbeweiser","===#","be honest","zerst√∂r","tasty human"),$post["message"]);
//	$post["name"]=str_ireplace(array("penis","schwanz","schw√§nze"," arsch","fuck","fick","pisse","drecksau","===D","jerk off","wichs","soggy,biscuit"),array("darij","darij","darijs"," kreisquadrierer","freu","freu","pizza"," Fermatbeweiser","===#","be honest","zerst√∂r","tasty human"),$post["name"]);	
...
//	$post['name']=substr($post['name'],0,100);
	...
	if (POST_LIMITS)
	{
        ...
        else
        {
			...
			{
	    		header ("HTTP/1.1 403 Forbidden");
				echo "Aus Traffic-Gr¸nden d¸rfen Posts momentan nicht allzu groﬂ sein.<br>" . $offset . " " . strlen ($post["message"]);
				die ();
			}
