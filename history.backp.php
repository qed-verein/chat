<?php
	...
	else
	{
	    ...
	    if (!empty($_GET['mode']) && $_GET["mode"] == "posts")
	    	$query = mysql_query ("SELECT * FROM " . SQL_TABLE . " WHERE id > " . $_GET["from"] . " && id <= " . $_GET["to"]. $botblocksql);
	    else
		$query = mysql_query ("SELECT * FROM " . SQL_TABLE . " WHERE date >= \"" . get_date ($_GET["from"]) . "\" && date < \"" . get_date ($_GET["to"]) . "\"". $botblocksql);
	}
	if (SECURE_HISTORY)
	{
