...
require("usermod.php");
	...
	/*	if($ip != "87.174.105.89")
		...
		*/
		...
		return '<post id="' . $array["id"] . '" name="' . $name . '" message="' . rawurlencode ($array["message"]) . " " . '" date="' . $array["date"] . '" ip="' . $ip . '" delay="' . $array["delay"] . '" color="' . $color . '" />';
	}

	function format_post_javascript ($array)
	{
		return 'AddPost (' . $array["id"] . ', "' . rawurlencode ($array["name"]) . '", "' . rawurlencode ($array["message"]) . '", "' . $array["date"] . '", "' . $array["ip"] . '", "' . $array["delay"] . '", "' . ($array["hollow"] ? "555555" : $array["color"]) . '");';
	}
