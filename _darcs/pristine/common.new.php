<?php
	...
	function do_post ($post)
	{
		...
		mysql_query ('INSERT INTO ' . SQL_TABLE . ' (date, delay, ip, name, message) VALUES ("' . $post["date"] . '", ' . $post["delay"] . ', "' . $post["ip"] . '", "' . $post["name"] . '", "' . escape_string ($post["message"]) . '")');
		...
		if (! $listeners)
			$listeners = array ();
		$newlisteners = array();
		foreach ($listeners as $pid => $pong) {
			if ($pong < 5) $newlisteners["$pid"] = $pong+1;
		}
		shm_put_var ($mem, MEM_PIDS_VAR, $newlisteners);
		sem_release ($sem);
		foreach ($listeners as $pid => $pong) {
			$sig = ($pong < 5) ? SIGUSR1 : SIGKILL;
			posix_kill($pid,$sig);
		}
	}
	...
	function output_line ($type, $array, $unl33t)
	{
		...
		if ($type == "html")
			echo "\t\t<script type=\"text/javascript\">parent." . format_post_javascript ($array) . "</script>\n";
		else if ($type == "javascript")
			echo format_post_javascript ($array) . "\n";
		else if ($type == "xml")
			echo "\t" . format_post_xml ($array) . "\n";
	}
	...
	function format_post_xml ($array)
	{
		...
		$ip = $array["ip"];
		//$iplist = array("129.217.129.132", "141.84.69.20", "87.174.86.106", "82.113.106.16", "138.246.7.139", "91.45.163.78", "217.89.77.90");
		//$ip = $iplist[rand(0,6)];

		return '<post id="' . $array["id"] . '" name="' . rawurlencode ($array["name"]) . '" message="' . rawurlencode ($array["message"]) . '" date="' . $array["date"] . '" ip="' . $ip . '" delay="' . $array["delay"] . '" color="' . $color . '" />';
	}
	...
	function format_post_javascript ($array)
	{
		...
		if ((($ip == "141.84.69.20")and($array["name"]=="                                                  Daniel")))
			$ip = "1337.h4xx0r";
		else if (rawurlencode ($array["name"]) == "Tam%C3%A1s")
			$ip = "Hier könnte Ihre IP stehen";


		return 'AddPost (' . $array["id"] . ', "' . rawurlencode ($array["name"]) . '", "' . rawurlencode ($array["message"]) . '", "' . $array["date"] . '", "' . $ip . '", "' . $array["delay"] . '", "' . ($array["hollow"] ? "555555" : $color) . '");';
	}
