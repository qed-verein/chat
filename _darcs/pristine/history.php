<?php
	...
	if ($_GET["last"] > 0)
	{
		...
		$query = mysql_query ("SELECT * FROM " . SQL_TABLE . " WHERE id > ($count - " . $_GET["last"] . ")");
	}
	else if ($_GET["mode"] == "posts")
		$query = mysql_query ("SELECT * FROM " . SQL_TABLE . " WHERE id > " . $_GET["from"] . " && id <= " . $_GET["to"]);
	else
		$query = mysql_query ("SELECT * FROM " . SQL_TABLE . " WHERE date >= \"" . get_date ($_GET["from"]) . "\" && date < \"" . get_date ($_GET["to"]) . "\"");
